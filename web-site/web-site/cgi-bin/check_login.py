...
check_user_id = 'SELECT * FROM login WHERE userName= %s'

cur = cnx.cursor()
cur.execute(check_user_id, (username,))
result = cur.fetchone()
cur.close()
...
if login:
    ...
    #get user info query
    get_user_info = "SELECT customerID,firstName,lastName,rewardsPoints,rewardLevel,points_until_flight FROM customers c JOIN rewards r ON r.rewardsID=c.rewardsID WHERE customerID=%s"
    user_info_cur = cnx.cursor()
    user_info_cur.execute(get_user_info,( result[2],))
    ...
    get_res = "SELECT reservationID, f.flightID, state, departureDay, departureTime, numberOnRes, arrivalTime, dep.cityName, dest.cityName FROM reservations r JOIN flights f ON r.flightID = f.flightID JOIN departures dep ON f.departureID = dep.departureID JOIN destinations dest ON dest.destinationID = f.destinationID WHERE r.customerID=%s"
    res_curs = cnx.cursor()
    res_curs.execute(get_res, ( result[2],))
